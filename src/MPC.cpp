...
public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N; i++) {
      fg[0] += cte_weight_in_cost*CppAD::pow(vars[cte_start + i] - ref_cte, 2);
      fg[0] += epsi_weight_in_cost*CppAD::pow(vars[epsi_start + i] - ref_epsi, 2);
      fg[0] += v_weight_in_cost*CppAD::pow(vars[v_start + i] - ref_v, 2);
    }
    
    // Minimize the use of actuators.
    for (int i = 0; i < N - 1; i++) {
      fg[0] += steer_weight_in_cost*CppAD::pow(vars[delta_start + i], 2);
      fg[0] += throttle_weight_in_cost*CppAD::pow(vars[a_start + i], 2);
    }

    // Minimize the value gap between sequential actuations.
    for (int i = 0; i < N - 2; i++) {
      fg[0] += steerchange_weight_in_cost*CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += throttlechange_weight_in_cost*CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
...
Solution MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (auto i = 0; i < N-1 ; i++){
    // cout << "solution.x[x_start+" << i <<  "]: " << solution.x[x_start+i] << "solution.x[y_start+" << i <<  "]: " << solution.x[y_start+i] << endl;
